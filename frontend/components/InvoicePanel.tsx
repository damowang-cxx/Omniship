...
export function InvoicePanel({ userId, admin = false }: { userId?: string; admin?: boolean }) {
  ...
  const total = useMemo(
    () => eligible.filter((item) => selected.includes(item.id)).reduce((sum, item) => sum + Number(item.amount), 0),
    [eligible, selected]
  );
